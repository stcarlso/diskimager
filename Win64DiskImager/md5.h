...
public:
	...
	// context.
	void Update(unsigned char *input, size_t inputLen) {
		unsigned int index;
		size_t partLen, i;
		...
		if (inputLen >= partLen) {
			...
			for (i = partLen; i + (size_t)63 < inputLen; i += (size_t)64)
	...
	void writeToString() {
		...
		for (pos = 0; pos < 16; pos++) {
			// Write to buffer
			sprintf_s(buffer, "%02x", digestRaw[pos]);
			memcpy(digestChars + (pos << 1), buffer, 2);
		}
		digestChars[32] = '\0';
	}
